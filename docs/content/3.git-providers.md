...
Studio supports three Git providers for repository operations: GitHub, GitLab, and Azure DevOps.
...
To use Azure DevOps as your Git provider, configure your repository settings in `nuxt.config.ts`:
...
export default defineNuxtConfig({
  studio: {
    repository: {
      ...
      owner: 'your-org', // Same as organization
      repo: 'your-repo',
      branch: 'main' // Optional, defaults to 'main'
    }
  }
...
::note
Azure DevOps requires both `organization` and `project` fields in addition to the standard repository configuration.
::

#### Creating an Azure DevOps Personal Access Token

::warning
Azure DevOps **only supports Personal Access Token (PAT) authentication**. OAuth is not currently available.
::

::prose-steps{level="4"}
#### Navigate to Azure DevOps Token Settings

Go to your Azure DevOps organization at `https://dev.azure.com/{organization}`, click **User Settings** (top right), then **Personal access tokens**.

#### Configure the Azure DevOps Token

Click **+ New Token** and fill in the required fields:
...
:warning[Copy the generated token immediately; you won't be able to see it again.]

For consistency, prefix your PAT with `az_pat_`:
...
::tip
For detailed Azure DevOps setup instructions, see the [Azure DevOps Setup Guide](https://github.com/nuxt-content/studio/blob/main/AZURE_DEVOPS_SETUP.md).
::
